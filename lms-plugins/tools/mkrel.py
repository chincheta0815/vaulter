...
def releaseUrl(version):
    return "https://raw.githubusercontent.com/chincheta0815/vaulter/master/lms-plugins/pkgs/%s-%s.zip" % (PLUGIN_NAME, version)
...
def updateInstallXml(version):
    ...
    with open(os.path.join(base_dir, installXml), "r") as f:
        lines=f.readlines()
    ...
    with open(os.path.join(base_dir, installXml), "w") as f:
        for line in lines:
            f.write(line)
...
def createZip(version):
    info("Creating ZIP")
    zipFile = "%s-%s" % (PLUGIN_NAME, version)
    shutil.make_archive(os.path.join(pkg_dir, zipFile), 'zip', os.path.join(base_dir, PLUGIN_NAME))
    zipFile += ".zip"
    return zipFile
...
def getSha1Sum(zipFile):
    ...
    sha1 = hashlib.sha1()
    with open(os.path.join(pkg_dir, zipFile), 'rb') as f:
        while True:
            data = f.read(65535)
            if not data:
                break
...
version = sys.argv[2]
base_dir = sys.argv[1]
pkg_dir = os.path.join(base_dir, 'pkgs')
if version != "test":
...
if version != "test" and os.path.exists(os.path.join(base_dir, REPO_XML)):
    updateRepoXml(os.path.join(base_dir, REPO_XML), version, zipFile, sha1, PLUGIN_NAME)
